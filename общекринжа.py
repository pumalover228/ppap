...
with open('80 mmHg.txt', 'r') as mm80:
    ...
    Sum = 0
    for i in range(10, len(temp80) - 1, 1):
        Sum += int(temp80[i])
...
with open('120 mmHg.txt', 'r') as mm120:
    ...
    Sum = 0
    for i in range(10, len(temp120) - 1, 1):
        Sum += int(temp120[i])
...
with open('160 mmHg.txt', 'r') as mm160:
    ...
    Sum = 0
    temp160_int = []
    for i in range(10, len(temp160) - 1, 1):
...
plt.title('Addiction between ADC counts and pressure', fontsize=7, color='black')
ax.plot(pressure_calib_float, adc_calib_int, color='r', linewidth=1)
plt.axis([35, 165, min(temp40_int) - 100, max(temp160_int) + 100])
plt.xlabel('Pressure, mm', fontsize=6)
plt.ylabel('ADC counts', fontsize=6)
...
graph = mlines.Line2D([], [], color='red', markersize=30, label='Pressure = Counts * k')
